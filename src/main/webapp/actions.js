...
function logActivity(activity, position, details, code, module) {
    ...
    if (typeof interactionlog != "undefined") {
        ...
        if (
            ...
            targetInteractions.indexOf(activity) != -1
        ) {
...
function CLSstr() {
    // get section number
    var navItem = $(".navitem.selected");
    var sectionNo = 1;
    if (navItem.length > 0 && navItem.attr("id")) {
        sectionNo =
            parseInt(navItem.attr("id").replace("navitem", "").trim()) + 1;
    }
...
function LOGtestStart(id, code, fake, medal) {
    var diff = fake
        ? "test overridden"
        : getLevenshteinDistance(lastcode, code);
    lastcode = code;
    lastid = id;
    if (id != "") id = ":" + id;
...
function cheatNotify(medalDetails, source) {
    var details =
        "Suspicious activity detected on NoobLab exercise\n\n" +
