...
function codePaste(e) {
    if (!$("div#code-main").is(":visible")) {
        apprise(
            "You cannot paste this text-based code with the blocks enabled."
        );
        return;
    }
    apprise(
        "Pasting this code into the editor will overwrite any existing code. Are you sure?",
        { verify: true },
        function (r) {
            if (r) {
                ...
                if (typeof NoobLabDesktop !== "undefined") {
                    NoobLabDesktop.setCode(actualCode);
                }
                ...
                if (editor.getValue().indexOf("NOOBLAB READONLY") != -1)
...
function getCarolDiv() {
    ...
    foundCarol.each(function () {
        ...
        if (
            ...
            (elementOffset.top + elementHeight > minTop &&
...
function buildCarolDiv(caroldiv) {
    if ($("div.parameter#kinder").text().trim() != "true") {
        $(caroldiv).click(function () {
            buildCarolDiv(this);
        });
    }
    var st = $("#content").scrollTop();

    var carolsize = $(caroldiv)
        .find("div.size")
        .css("display", "none")
        .text()
        .trim();
    ...
    var surroundingDiv = $(
        '<div class="carolwrapper" style="border: 1px solid gray; height: 100%; width: 100%"></div>'
    );
    var pickupNo = 1;

    var carolwidth = $(window).height() * 0.6;
    if (carolwidth > $("div#content").width())
        carolwidth = $("div#content").width() * 0.8;
    carolwidth = parseInt(carolwidth);

    if ($(caroldiv).attr("data-size") != undefined)
        carolwidth = $(caroldiv).attr("data-size");

    // set up the board.
    for (var y = 0; y < carolsize; y++) {
        for (var x = 0; x < carolsize; x++) {
            var square = $(
                '<div class="carolsquare carolpos' + x + "-" + y + '"></div>'
            );
    ...
    $(caroldiv)
        .find(".carolwrapper")
        .css({ width: carolwidth + "px", height: carolwidth + "px" });
    ...
    if ($("div.parameter#kinder").text().trim() == "true") {
        ...
        if (
            $("div.parameter#kinderlevel").text().trim() != "" &&
            $("div.parameter#kinderlevel").text() >= 2
        ) {
            bar.append(
                '<img class="go" src="' + contextPath + '/images/go.png"/>'
            );
        }
        ...
        var arrowWidth = $(caroldiv)
            .find("div.kinderbuttons img")
            .eq(0)
            .outerWidth();
        ...
        var programbar = $(
            '<div style="position: relative" class="programbar">'
        );
        programbar.css({
            width: arrowWidth + "px",
        });
...
function kinderGloriousWinTestCode() {
    ...
    if (
        $("div.parameter#kinderlevel").text().trim() == "" ||
        $("div.parameter#kinderlevel").text().trim() >= 2
    ) {
        win +=
            "else { parent.unhappyBilly(); hold(1000); carol.updateCarol(true); }";
    }
    return win;
}
...
function kinderCarolCode(source) {
    ...
    if (
        ...
        code +=
            "try { DIRECTION(); } catch (e) { parent.unhappyBilly(); hold(1000); carol.updateCarol(true); parent.$('.programbar').empty(); }\n";
...
function updateCarolProgramBar(direction) {
    ...
    var newEntry = $(
        '<div style="white-space: nowrap" class="programline"></div>'
    );
    newEntry.append(
        '<img data-direction="' +
            direction +
    ...
    newEntry.append(
        '<div class="quantity" style="display: inline-block">1</div>'
    );
    ...
    $(currentCarol)
        .find(".programbar")
        .animate(
            { scrollTop: $(currentCarol).find(".programbar")[0].scrollHeight },
            "slow"
        );
}
...
function createEmos() {
    ...
    $("div.emo").each(function () {
        ...
        howDoYouFeelAbout(
            this,
            "<b>How do you feel about this workshop at the moment?</b>",
            emodesc,
            "...and click the icon that best matches your emotion right now:",
            true
        );
...
function createFakeDocs() {
    ...
    $fakedoc.each(function () {
        ...
        $(this)
            .prepend(
                '<iframe style="height: 1px" allowTransparency="true" frameborder="0" class="fakedoc"></iframe>'
            )
            .ready();
        var self = this;
        ...
        $(this)
            .parent()
            .prepend(
                '<div><span class="fakedoctab selected render">Original</span><span class="fakedoctab source">HTML</span>'
            );
        $(this)
            .parent()
            .append('<div class="htmleditor"><textarea>xxxx</textarea></div>');
        $(this).parent().find("textarea").val(html.trim());
        //; turn textarea into codemirror editor
        var editor = CodeMirror.fromTextArea(
            $(this).parent().find("textarea")[0],
            {
                mode: "text/html",
                tabMode: "indent",
        ...
        $(this)
            ...
            .click(function () {
                ...
                var bodyelement = $(
                    $(self).find("iframe")[0].contentWindow.document
                ).find("body")[0];
...
function updateVisibleFakeDoc() {
    ...
    $("div.fakedocwrapper").each(function () {
        ...
        $(this).find("iframe")[0].contentWindow.document.body.onclick =
            function () {
                $("div.fakedocwrapper div.fakedoc").css(
                    "border",
                    "1px solid black"
                );
                $("div.fakedocwrapper").removeClass("onscreen");
                $(that).addClass("onscreen");
                $("div.fakedocwrapper.onscreen")
                    .find("div.fakedoc")
                    .css("border", "4px solid black");
                visibleFakeDoc = $("div.fakedocwrapper.onscreen").find(
                    "iframe"
    ...
    if ($("div.fakedocwrapper.onscreen").size() == 1) {
        $("div.fakedocwrapper.onscreen")
            .find("div.fakedoc")
            .css("border", "4px solid black");
        visibleFakeDoc = $("div.fakedocwrapper.onscreen").find("iframe")[0]
...
function createQuickQuizzes() {
    ...
    $(".qqmain, .qqinput").each(function () {
        ...
        if ($(this).hasClass("qqmain")) {
            $(this).prepend(
                '<div style="padding: 4px; background-color: black; color: #EDF4F5; margin-bottom: 0.5em">Quiz Question <span class="qno">' +
                    qno +
                    "</span> (click your answer)</div>"
            );
            $(this)
                .find(".response")
                .each(function () {
            ...
            $(this).prepend(
                '<div style="padding: 4px; background-color: black; color: #EDF4F5; margin-bottom: 0.5em">Quiz Question <span class="qno">' +
                    qno +
                    "</span> (enter your answer and press return)</div>"
            );
            var answer = $(this)
                .find(".correct")
                .clone()
            ...
            $(this)
                ...
                .eq(0)
                .before(
                    '<br/><input type="text" onkeydown="if (event.keyCode == 13) handleQQInput(this,\'' +
                        answer +
                        "')\"/>"
                );
    ...
    $("div.hiddenRun").each(function () {
        ...
        $("<div></div>")
            ...
            .text(
                ">>> Click here to see an example of what your program should do <<<"
            )
            .insertBefore($(this));
        $(this).remove();
    });
...
function handleTestCasesBasic() {
    $(".testCase").each(function () {
        ...
        var func = function () {
            ...
            tests.each(function () {
                var finalOutput = $(this)
                    .find(".testFinalOutput")
                    .text()
                    .trim();
                var finalOutputJS = $(this)
                    .find(".testFinalOutputJS")
                    .text()
                    .trim();
                ...
                for (
                    ...
                    var answerDocument =
                        $("#outputframe")[0].contentWindow.document;
                    var answerText = answerDocument
                        .getElementById("output-main")
                        .innerHTML.trim();
                    ...
                    if (finalOutput != "") {
                        ...
                        if (finalOutput.charAt(0) != "!") {
                            if (answerText.indexOf(finalOutput) != -1)
                                successfulTests++;
                        } else {
                    ...
                    if (codeIncludes != "") {
                        ...
                        if (codeIncludes.charAt(0) != "!") {
                            if (codeInEditor.indexOf(codeIncludes) != -1)
                                successfulTests++;
                        } else {
                            if (codeInEditor.indexOf(codeIncludes) == -1)
                                successfulTests++;
                ...
                if (numberOfTests == successfulTests) {
                    ...
                    var htmlToAdd =
                        ...
                        "Sorry! Your program did not produce the expected output! It passed " +
                        successfulTests +
                        " out of " +
                        numberOfTests +
                        " test(s). Check your work and try again!</div>";
                    objDiv.innerHTML = htmlToAdd;
...
function howDoYouFeelAbout(
    ...
    if (
        $(source).find("div.emotionselection div.message").text().trim() ==
        message
    )
        return;
    ...
    var emotiondiv = $(
        '<div class="emotionselection" style="margin-top: 0.8em"></div>'
    );

    emotiondiv.append(
        '<div class="message" style="margin-bottom: 0.8em; margin-top: 0.8em">' +
            message +
            "</div>"
    );
    ...
    emotiondiv.append(
        '<div class="imgwrapper" style="display: inline-block; width: 44px; font-size: 60%; color: transparent"><div style="position: relative;">Happy</div><img src="' +
            contextPath +
            '/images/happy.jpg"/></div>'
    );
    emotiondiv.append(
        '<div class="imgwrapper" style="display: inline-block; width: 44px; font-size: 60%; color: transparent"><div style="position: relative;">Surprised</div><img src="' +
            contextPath +
            '/images/surprise.jpg"/></div>'
    );
    emotiondiv.append(
        '<div class="imgwrapper" style="display: inline-block; width: 44px; font-size: 60%; color: transparent"><div style="position: relative;">Afraid</div><img src="' +
            contextPath +
            '/images/fear.jpg"/></div>'
    );
    emotiondiv.append(
        '<div class="imgwrapper" style="display: inline-block; width: 44px; font-size: 60%; color: transparent"><div style="position: relative;">Disgust</div><img src="' +
            contextPath +
            '/images/disgust.jpg"/></div>'
    );
    emotiondiv.append(
        '<div class="imgwrapper" style="display: inline-block; width: 44px; font-size: 60%; color: transparent"><div style="position: relative;">Angry</div><img src="' +
            contextPath +
            '/images/anger.jpg"/></div>'
    );
    emotiondiv.append(
        '<div class="imgwrapper" style="display: inline-block; width: 44px; font-size: 60%; color: transparent"><div style="position: relative;">Unhappy</div><img src="' +
            contextPath +
            '/images/unhappy.jpg"/></div>'
    );

    emotiondiv.append(
        '<div class="imgwrapper" style="display: inline-block; width: 44px; font-size: 60%; color: transparent"><div style="position: relative;">Other</div><img src="' +
            contextPath +
            '/images/other.png"/></div>'
    );

    emotiondiv.append("<div>Type any extra thoughts below...</div>");
    emotiondiv.append(
        '<textarea class="emothoughts" style="width: 90%; height: 5em; font-family: sans-serif"></textarea>'
    );
    ...
    $(emotiondiv)
        ...
        .click(function (e) {
            ...
            if (emotion != "other") {
                ...
                apprise(
                    "Please enter a single word to describe how you feel.",
                    { input: true },
                    function (r) {
                        if (r) {
                            emotion = "*" + r;
            ...
            function doIt() {
                ...
                if (recordThoughts) {
                    emotion +=
                        ":" +
                        $(that)
                            .closest("div.emotionselection")
                            .find("textarea.emothoughts")
                            .val();
                ...
                $(that)
                    .closest("div.emotionselection")
                    .find("div.message2")
                    .text("You said you felt");
                setTimeout(function () {
                    $(source)
                        .find("div.emotionselection")
                        .hide(1000, function () {
                            $(source).find("div.emotionselection").remove();
                        });
                    $.colorbox.close();
                }, 1000);
                e.stopPropagation();
    ...
    if ($(source).hasClass("emo")) {
    ...
    } // it's something that's been popped up due to an event, e.g. medal win/fail
    else {
        ...
        if (shortmsg.indexOf("medal") != -1) {
            ...
            emotiondiv.prepend(
                '<p style="font-size: 130%; font-weight: bolder"><img class="notransparent" style="vertical-align: middle; height: 70px" src="' +
                    contextPath +
                    "/images/medal" +
                    shortmsg +
                    '.png"/> You won a ' +
        ...
        if (shortmsg.indexOf("repeatedFail") != -1) {
            ...
            emotiondiv.prepend(
                '<p style="font-size: 130%; font-weight: bolder"><img class="notransparent" style="vertical-align: middle; height: 70px" src="' +
                    contextPath +
                    '/images/failed.png"/> Repeated activity failure</p>'
            );
        }
...
function medalGreatSuccess(medal, fb, id) {
    if (typeof fb === "object") {
        ...
        for (var i = 0; i < fb.length - 1; i++) {
            fbstr =
                fbstr + '<div style="margin-bottom: 1em">' + fb[i] + "</div>";
        }
    ...
    $("body", iframeDoc).append(
        '<div id="testresult" style="max-height: calc(100vh - 20px); overflow: auto; font-family: monospace; font-weight: bold; position: fixed; border: 2px solid black; background-color: white; top: 10px; padding : 10px; width: 95%; box-sizing: border-box; color: green">' +
            fb +
            "Well done! Your work passed the test!</div>"
    );
    ...
    if (medal) {
        ...
        var medalType =
            medalData[0] == "ribbon" ? medalTypeOnly : medalTypeOnly + " medal";
    }
    if (medal) {
...
function medalEpicFail(SUCCESSFULTESTS, NUMBEROFTESTS, fb) {
    ...
    $("body", iframeDoc).append(
        '<div id="testresult" style="max-height: calc(100vh - 20px); overflow: auto; font-family: monospace; font-weight: bold; position: fixed; border: 2px solid black; background-color: white; top: 10px; padding : 10px; width: 95%; box-sizing: border-box; color: red">' +
            fb +
            "Sorry! Your work did not produce what we were looking for!</div>"
    ); // It passed '+SUCCESSFULTESTS+' of '+NUMBEROFTESTS+' test(s).</div>');
    $("div#testresult", iframeDoc).css("opacity", 0.8);
...
function getStyle(className) {
    ...
    for (sheets = document.styleSheets.length - 1; sheets >= 0; sheets--) {
        classes =
            document.styleSheets[sheets].rules ||
            document.styleSheets[sheets].cssRules;
...
function handleTestCasesHtml() {
    $(".testCase").each(function () {
        ...
        var func = function (e) {
            ...
            // feedback
            code +=
                'var fb = ""; function feedback(newFb) { fb += "<p style=\\"margin-top: 0px\\">"+newFb+"</p>" };';
            ...
            tests.each(function () {
                var finalOutputJS = $(this)
                    ...
                    .text()
                    .trim();
                var finalOutputPHP = $(this)
                    .find(".testFinalOutputPHP")
                    .text()
                    .trim();
                var codeIncludes = $(this).find(".codeIncludes").text().trim();
                var repeat = $(this).attr("data-repeat");
                ...
                if (repeat) {
                    // add a for loop on the beginning
                    oneCheck =
                        "\nfor (var flibble1234567 = 0; flibble1234567 < " +
                        repeat +
                        "; flibble1234567++)\n" +
                        oneCheck;
                }
                ...
                if (finalOutputPHP) {
                    code = code.replace(
                        "/* PHPFEEDBACKHERE */",
                        "<?php function feedback($newFb) { echo 'feedback(\"'.$newFb.'\");'; } ?>"
                    );
                    oneCheck +=
                        "\n" +
                ...
                if (codeIncludes != "") {
                    ...
                    if (codeIncludes.charAt(0) != "!") {
                        if (codeIncludes.indexOf('"') == -1) {
                            oneCheck +=
                                "\n" +
                                'if (parent.editor.getValue().indexOf("' +
                                codeIncludes +
                                '") != -1) SUCCESSFULTESTS++;' +
                                "\n";
                        } else {
                ...
                if (endTestJS) {
                    ...
                    oneCheck += "\nNUMBEROFTESTS++;\n";
                    oneCheck +=
                        "var finalFunc = function () { " + endTestJS + " };\n";
        ...
        if (medal09876 != undefined) {
            linkText = linkText.replace(
                "code",
                "code for a " + medal09876.split(":")[0] + " medal"
            );
...
';
            ...
            tests.each(function (testno) {
                var finalOutput = $(this)
                    ...
                    .text()
                    .trim();
                var finalOutputCode = $(this)
                    .find(".testFinalOutputCode")
                    .text()
                    .trim();
                var codeIncludes = $(this).find(".codeIncludes").text().trim();
                ...
                var repeat = $(this).attr("data-repeat");
                var initcond = $(this).attr("data-initcond");
                if (!initcond && $(this).find(".testInit").text().trim() != "")
                    initcond = $(this).find(".testInit").text().trim();
                ...
                if (
                    $("div.parameter#language").text().trim() == "pythoncarol"
                ) {
                ...
                codeInEditor +=
                    "  noobdata['errorsDuringTest'].append(str(e))\n\n";
                if (runtimeCatch == "") {
                    codeInEditor +=
                        "  noobtest.feedback('Your code caused an error during the test. If you do not get an error when you run your code, your code probably does something completely unexpected for this particular exercise.')\n";
                } else {
                ...
                if (finalOutput != "") {
                    ...
                    finalOutputCode =
                        ...
                        "\nexcept Exception as e:\n" +
                        indent(
                            "noobtest.debug('Runtime error during test run: '+str(e))"
                        );
                    if (pythonFailCheck)
                        finalOutputCode = "return False\n" + finalOutputCode;
                    oneCheck +=
                        "\n" +
                        "def noobTestRun():\n" +
                        indent(finalOutputCode) +
                ...
                if (codeIncludes != "") {
                    ...
                    if (codeIncludes.charAt(0) != "!") {
                        oneCheck +=
                            "\n" +
                            'if "' +
                            codeIncludes.toLowerCase() +
                            '" in noobtest.getCode(): noobdata["successfulTests"] = noobdata["successfulTests"] + 1' +
                            "\n\n";
                    } else {
                        codeIncludes = codeIncludes.substr(1);
                        oneCheck +=
                            ...
                            'if "' +
                            codeIncludes.toLowerCase() +
                            '" not in noobtest.getCode(): noobdata["successfulTests"] = noobdata["successfulTests"] + 1' +
                            "\n\n";
                    }
                }
            ...
            setTimeout(function () {
                runPython("::istest::" + code, true, inputTestList);
            }, 1);
        },
        function (
...
function handleTestCasesJS() {
    $(".testCase").each(function () {
        ...
        var func = function (e) {
            ...
            code +=
                'function liveOutput() { return (document.getElementById("output-main").innerHTML+"").trim().replace(/<br.*?>/gi,"\\n") }';
            ...
            tests.each(function () {
                ...
                var codeInEditor = editor.getValue();
                codeInEditor += "\n\n";
                codeInEditor = codeInEditor.replace(
                    /document/g,
                    "parent.visibleFakeDoc"
                );

                if (
                    $("div.parameter#language").text().trim() == "pcode" ||
                    $("div.parameter#language").text().trim() == "pcarol"
                ) {
                    codeInEditor = pcodeToJs(codeInEditor);
                }

                if (
                    $("div.parameter#language").text().trim() == "carol" ||
                    $("div.parameter#language").text().trim() == "pcarol"
                ) {
                ...
                codeInEditor =
                    "try { " +
                    codeInEditor +
                    ' } catch (e) { feedback("Your code caused an error during the test: "+e.message); errorsDuringTest.push(e.message); }';
                ...
                if (finalOutput != "") {
                    ...
                    oneCheck +=
                        "\n" +
                        "var testFunc = function(finalOutput) { " +
                        finalOutputJS +
                        " };";
                    oneCheck +=
                        "\n" +
                ...
                if (codeIncludes != "") {
                    ...
                    if (codeIncludes.charAt(0) != "!") {
                        if (codeIncludes.indexOf('"') == -1) {
                            oneCheck +=
                                "\n" +
                                'if (parent.editor.getValue().toLowerCase().indexOf("' +
                                codeIncludes.toLowerCase() +
                                '") != -1) SUCCESSFULTESTS++;' +
                                "\n";
                        } else {
...
function restoreMedalCode(e) {
    ...
    if ($("#code-blockly").is(":visible")) {
        apprise(
            "Sorry! You can't retrieve your previous code when you're using the block-based editor."
        );
        return;
    }
    var url =
        contextPath + "/stats?type=lastmedal&testid=" + $(this).attr("data-id");
    if ($(this).attr("data-medal"))
        url += "&medal=" + $(this).attr("data-medal");
    var message =
        "This will overwrite the code in the editor with the code you used to XYZXYZ. Are you sure?";
    if ($(this).attr("data-medal")) {
        ...
        $.ajax({
            ...
            success: function (r) {
                ...
                if (getUrlParameter("embedmedal")) {
                    ...
                    if (
                        $("div.testCase").eq(0).attr("data-medal") !=
                            undefined &&
                        $("div.testCase").eq(0).attr("data-medal") != ""
                    ) {
                        ...
                        if (dm.indexOf("bronze") != -1) {
                            $("img#embedmedalhighlight").attr(
                                "src",
                                src.replace("gold", "bronze")
                            );
                        } else if (dm.indexOf("silver") != -1) {
                            $("img#embedmedalhighlight").attr(
                                "src",
                                src.replace("gold", "silver")
                            );
                        }
                    ...
                    if (medals[testId] != undefined) {
                        ...
                        if (medal == targetMedal) {
                            ...
                            if (medal != "") {
                                ...
                                html =
                                    '<p style="margin: 0px; padding: 0px; text-align: center">You have already won ';
                                ...
                                html =
                                    ...
                                    html;
                                html +=
                                    "<br/>&nbsp;<br/>&gt;&gt;&gt Click here to repeat the test for a " +
                                    targetMedal +
                                    ' medal &lt;&lt;&lt</p><div style="clear: both"></div>';
                                ...
                                if (medal != "ribbon") {
                                    ...
                                    ass.html(
                                        "(Did another student help you with this exercise? Click <span>here</span> to grant an assist)"
                                    );
                                    ass.find("span")
                                        .css({
                                            cursor: "pointer",
                                    ...
                                    if (
                                        assists[testId + ":awarded"] !=
                                        undefined
                                    ) {
                                        var points =
                                            assists[testId + ":awarded"];
                                        $(ass).append(
                                            '<div style="text-align: right">&nbsp;<br/>Existing assist(s) awarded; points deducted: ' +
                                                points +
                                                "</div>"
                                        );
                                    }
                                ...
                                html =
                                    "&gt;&gt;&gt You've passed this already - click to repeat the test &lt;&lt;&lt";
                            }
                            var reloadCode = $(
                                '<div class="reloadcode" style="position: absolute; top: 2px; right: 10px; cursor: pointer; font-size: 130%"><i class="fa fa-file-code-o" aria-hidden="true"></i></div>'
                            );
                            $(this)
                                .contents()
                                .not("div.emotionselection,span.override")
                                .remove();
                            $(this).prepend(html);
                            $(this).append(reloadCode);
                            reloadCode.attr("data-id", testId);
                            if (medal != "")
                                reloadCode.attr("data-medal", medal);
                            reloadCode.click(restoreMedalCode);
                        } else if (medal != "") {
                            var greater = false;
                            if (
                                targetMedal == "bronze" &&
                                $.inArray(medal, ["silver", "gold"]) != -1
                            )
                                greater = true;
                            if (targetMedal == "silver" && medal == "gold")
                                greater = true;
                            var msg =
                                "You currently have a *MEDAL* medal for this exercise.";
                            if (greater)
                                msg =
                                    "You already have a better (*MEDAL*) medal for this exercise.";
                            ...
                            if (greater)
                                html =
                                    ...
                                    html;
                            html +=
                                "<br/>&nbsp;<br/>&gt;&gt;&gt Click here to test for the " +
                                targetMedal +
                                ' medal &lt;&lt;&lt</p><div style="clear: both"></div>';
                            html = html.replace("ribbon medal", "ribbon"); // shouldn't ever happen, but hey
                            $(this)
                ...
                $("div.section").each(function (index) {
                    var testcases = $(this)
                        .find(".testCase[data-medal]")
                        .size();
                    if (testcases != 0) {
                        var completed = $(this)
                            .find(".testCase.completed[data-medal]")
                            .size();
                        var wonmedals = $(this)
                            .find(".testCase.completed.medalwon[data-medal]")
                            .size();
                        if (completed == 0)
                            $(".navitem[id]")
                                .eq(index)
                        ...
                        if (completed != 0)
                            $(".navitem[id]")
                                ...
                                .find("a i")
                                .css("color", "#E68332");
                        if (completed == testcases)
                            $(".navitem[id]")
                                .eq(index)
                                .find("a")
                                .html(
                                    '<i style="color: #777777" class="fa fa-check-circle"></i>'
                                );
                        if (wonmedals == testcases)
                            $(".navitem[id]")
                                .eq(index)
                                .find("a")
                                .html(
                                    '<i style="color: #7FA859" class="fa fa-check-circle"></i>'
                                );
                    }
                });
                ...
                $.ajax({
                    ...
                    success: function (results) {
                        for (var key in results) {
                            ...
                            var targetSearch =
                                '.testCase[data-id="' + key + '"]';
                            if (targetMedal) {
                                targetSearch +=
                                    '[data-medal^="' + targetMedal + '"]';
                            }
                            var target = $(targetSearch);
                            target.attr("data-globalattempts", result);
                            target.each(function () {
                                var maxattempts =
                                    $(this).attr("data-maxattempts");
                                if (isNaN(maxattempts) || result == -1)
                                    maxattempts = 0;
                                var maxdowngrade =
                                    $(this).attr("data-maxdowngrade");
                                if (result >= maxattempts && maxattempts != 0) {
                                    ...
                                    if (
                                        medalNums[thismedal] >
                                        medalNums[maxdowngrade]
                                    ) {
                                        $(this)
                                            .contents()
                                            .eq(0)
                                            .replaceWith(
                                                '<span style="color: gray">You have exceeded the maximum number of attempts. You can now only win the ' +
                                                    maxdowngrade +
                                                    " medal for this exercise.</span>"
                                            );
                                        $(this).unbind("click");
                                        $(this).click(function (e) {
                                            if (e.shiftKey)
                                                $(this)
                                                    .find("button,br,input")
                                                    .show();
                                        });
                                    }
...
function assist(testId, medal) {
    var msg =
        '<div style="font-family: Verdana, sans-serif; font-size: 90%"><h2 style="margin-top: 0px">Grant an assist</h2>';
    var maxpoints = 1;
    ...
    if (medal == "bronze") {
        msg +=
            "<p>This is a bronze medal and worth <b>2 NoobLab points</b>.</p>";
        msg +=
            "<p>You can grant only one assist for this medal and you<br/>will be awarding <b>1 of your 2 points</b> for the assist.";
    }

    if (medal == "silver") {
        maxpoints = 2;
        msg +=
            "<p>This is a silver medal and worth <b>3 NoobLab points</b>.</p>";
        if (previouslyawarded != 0) {
            ...
            msg +=
                '<input type="radio" name="assistpoints" value="1" checked> 1 point<br/>';
            msg +=
                '<input type="radio" name="assistpoints" value="2"> 2 points<br/>';
        }
    }
    ...
    if (medal == "gold") {
        ...
        if (previouslyawarded == 2) {
            msg +=
                "<p>You have already awarded two points for this activity<br/>";
            msg += "and can only award one more point.</p>";
        } else if (previouslyawarded == 1) {
            msg +=
                "<p>You have already awarded one point for this activity.<br/>";
            ...
            msg +=
                "<p>You can award one, two or three points to the person<br/>";
            ...
            msg +=
                '<input type="radio" name="assistpoints" value="2"> 2 points<br/>';
            msg +=
                '<input type="radio" name="assistpoints" value="2"> 3 points<br/>';
        }
    }

    msg +=
        "<p>Please type the K number of the person you want<br/>to grant the assist to below.</p>";
    msg +=
        "<p><i>Be sure that you get the K number correct!<br/>You cannot undo this afterwards!</i></p>";

    if (previouslyawarded == maxpoints) {
        apprise(
            "You have already given away all the assist points available for this exercise."
        );
        return;
    }
    ...
    apprise(msg, { input: true }, function (r) {
        if (r) {
            ...
            if (kno.charAt(0) != "k") {
                alert(
                    "You must enter a K number that starts with the letter K."
                );
                return;
            }
            // otherwise
    ...
    $("#code-titlebar div.tab")
        ...
        .each(function (i, tab) {
            ...
            var tabname = $(tab)
                ...
                .trim();
            if (
                (tabname.slice(-4) == ".htm" || tabname.slice(-5) == ".html") &&
                injectIntoIndex &&
                injectIntoIndex.indexOf("?php") == -1
            ) {
...
function saveState(s) {
    ...
    if (sectionNo !== lastSaveStateSectionNo) {
        $.cookie("nlpp-" + courseNo + "-" + lessonNo + "-lastnav", sectionNo, {
            expires: 365,
            path: "/",
        });
    ...
    if (code != lastSaveStateCode) {
        $.cookie("nlpp-" + courseNo + "-" + lessonNo + "-code", code, {
            expires: 365,
            path: "/",
        });
    ...
    if ($("div.parameter#blockly").text().trim() == "true") {
        ...
        $.cookie(
            "nlpp-" + courseNo + "-" + lessonNo + "-codeblockly",
            blocklyxml,
            { expires: 365, path: "/" }
        );
    } else {
        $.cookie("nlpp-" + courseNo + "-" + lessonNo + "-codeblockly", "", {
            expires: 365,
            path: "/",
        });
    }
...
function sectionLevelBlocklyVisibility() {
    ...
    if ($("div.section.selected").hasClass("noblockly")) {
        ...
        var original = $("div.parameter#blockly").attr(
            "data-original",
            original
        );
    ...
    $.post(
        ...
        function (result) {
            ...
            if (result == "success") {
                ...
                if (input) {
                    $(form).append(
                        '<textarea style="width:0px;height:0px;visibility:hidden" name="inputbuffer">' +
...
function hiddenRun(code, test, codefortest) {
    ...
    if (codefortest != undefined)
        codefortest = codefortest.replace(/console\.log/g, "println");
    ...
    if (
        ...
        for (var i = 0; i < carolLanguage.length; i++) {
            ...
            code = code.replace(
                re,
                "func = function(a,b,c,d,e) { try { return carol.$1; } catch (e) { throw new Error(e.message); } }()  "
            );
        }
    ...
    // pcode
        ...
        $("div.tab")
            ...
            .each(function () {
                var title = $(this).text();
                if (title.indexOf(".") == -1)
                    $(this).replaceWith(title + ".cpp");
                //if (title.slice(-4) != ".cpp") $(this).replaceWith(title+".cpp");
            });
        runCPP(getTabBundleCode(true));
    } else if (
        $("div.parameter#language").text().trim() == "python" ||
        $("div.parameter#language").text().trim() == "pythoncarol"
    ) {
        if ($("div.parameter#language").text().trim() == "pythoncarol") {
            try {
                if (code.toLowerCase().indexOf("carol.") != -1)
                    carol.initialiseCarol();
            } catch (e) {
                apprise(
                    "You need to have a maze visible on the screen to run a Carol program. Scroll the screen so that a maze is visible and is selected with a thick black border around it, and try again."
                );
        ...
        setTimeout(function () {
            runPython(code);
        }, 1); // not sure why the setTimout is needed for Carol following the initialise, but hey...
    } // plain ol' JS
    else {
        var $fakedoc = $("div.fakedoc");
        if ($fakedoc.length != 0) {
            ...
            if (
                $("div.fakedocwrapper.onscreen").size() == 0 &&
                code.indexOf("document") != -1
            ) {
                alert(
                    "You don't seem to have an HTML fragment on the screen to run your code against. Make sure you scroll the page so that the correct HTML fragment for your exercise is visible before running your code."
                );
                return;
            }
            if (
                $("div.fakedocwrapper.onscreen").size() > 1 &&
                code.indexOf("document") != -1
            ) {
                alert(
                    "You have two HTML fragments on the screen at once. Select which one you want to run your code against by clicking it before hitting the run button."
                );
                return;
            }
        }
        ...
        code = code.replace(
            /\.onclick\s*\=\s*(.*)\s*;/g,
            ".onclick = function() { try { $1.call(this) } catch (e) { logError(e) } };"
        );
...
function runFullWeb(code, logcode) {
    ...
    if (filename.slice(0, 5) != "index") {
        ...
        if (hasIndex) {
            apprise(
                "Do you want to start on the index page or the currently selected page?",
                {
                    verify: true,
                    textNo: "Use index page",
                    textYes: "Use " + filename,
                },
                function (r) {
    ...
    function goForIt() {
        ...
        for (var i = 0; i < codefiles.length; i++) {
            ...
            if (error && $("div.parameter#disableValidation") != "true") {
                ...
                $("body", outputframe.document).append(
                    '<code style="display: block; padding: 0.5em; background: white; border: 2px solid black; color: red"></code>'
                );
                $("body code", outputframe.document).text(
                    "Invalid HTML on line " +
                        error.lineno +
                        ", column " +
                        error.colno +
                        " - " +
                ...
                if (!$("#code-titlebar div.tab").eq(i).hasClass("selected"))
                    $("#code-titlebar div.tab").eq(i).click();
...
function save(tabs) {
    ...
    if (tabs) {
        watermarkedCode = watermarkedCode.replace(
            /\*\*\*TAB\*\*\*\s+\n/g,
            "***TAB***\n"
        );
        watermarkedCode = watermarkedCode.replace(
...
function clearEditor() {
    apprise(
        "Are you sure you want to clear the code pane? If you have not saved your work, you will lose any code currently entered.",
        { verify: true },
        function (r) {
            if (r) {
                editor.setValue("");
...
function stop() {
    if ($("div.parameter#language").text().trim() == "javaserver") {
        ...
        if (!newdoppio) {
            ...
            try {
                outputframe.controller.commandHandle("fnar");
            } catch (e) {}
        ...
        if (cppcompiling == true) {
            ...
            enableRun();
            cout(
                "\nCompilation terminated with the 'STOP' button.",
                " rgb(255,0,0)"
            );
        } else {
            ...
            enableRun();
            cout(
                "\nProgram terminated with the 'STOP' button.",
                " rgb(255,0,0)"
            );
        }
...
function postRun() {
    if ($(".testCase[data-fails=5]").length != 0) {
        ...
        howDoYouFeelAbout(
            $lastfail,
            "You've been unsuccessful at this activity five in times in a row now...",
            "repeatedFail"
        );
    }
    setTimeout(function () {
    ...
    if ($("div.parameter#multi").text().trim() == "true") {
        $("div.codebundle").each(function (i, source) {
            ...
            $(this).append(
                '<img src="' +
                    contextPath +
                    '/images/bundle_icon.png" style="position: absolute; top: -10px; left: 5px"/>'
            );
            $(this).append(
                '<div style="margin-left: 75px">This is a code bundle.<br/><b>Click to paste the code into the editor &gt;&gt;&gt;</b></div>'
            );
            $(this).css("cursor", "pointer");
            $(this).click(function () {
                pasteCodeBundle(source);
            });
        });
    }
}
...
function blockPaste(source) {
    if ($("div.parameter#blockly").text().trim() != "true") {
        apprise(
            "You cannot paste these example blocks with the text-based editor enabled."
        );
        return;
    }
    var code = $(source).text();
    apprise(
        "This will overwrite all of your existing blocks on the canvas! Are you sure?",
        { verify: true },
        function (r) {
            if (r) {
                restoreBlockly(code);
...
function pasteCodeBundle(source) {
    ...
    apprise(
        "Pasting this code bundle into the editor will overwrite any existing code. Be sure to save ALL your work first! Use the <i>Save all as zip</i> if you're slightly unsure!<p>Are you sure you want to proceed?</p>",
        { verify: true },
        function (r) {
            if (r) {
                ...
                $(source)
                    ...
                    .each(function () {
                        ...
                        addNewTab(true);
                        $("div.tab.selected").html(
                            tabname +
                                '<i class="close fa fa-times" onclick="deleteSelectedEditorTab()"></i>'
                        );
                        editor.setValue(code);
                        javaPidjinCodeWrapper(true);
                    });
...
function selectEditorTab(source, norename) {
    if (!$(source).hasClass("selected")) {
        ...
        $("#code-titlebar div.tab.selected")
        ...
        apprise(
            "What do you want to rename the tab to?",
            { input: true },
            function (r) {
                if (r) {
                    if (r.split(".").pop() == r) {
    ...
    function doTheRest() {
        ...
        if (
            name.slice(-4) == ".cpp" ||
            $("div.parameter#language").text().trim() == "cpp"
        ) {
            editor.setOption("mode", "text/x-c++src");
            $("input#tidy").prop("onclick", null);
            $("input#tidy").unbind("click");
            $("input#tidy").click(function () {
                tidyCode();
            });
            $("input#tidy").hide();
            $("input#tidy").val("Tidy");
...
function deleteSelectedEditorTab() {
    ...
    apprise(
        "Do you really want to remove this tab and any code within it? If you have not saved it, any code will be lost!",
        { verify: true },
        function (r) {
            if (r) {
                if ($("#code-titlebar div.tab").not(".newtab").length < 2) {
                    ...
                    javaPidjinCodeWrapper(true);
                    if (
                        $("div.parameter#language").text().trim() == "fullweb"
                    ) {
                        $("div.tab.selected").html(
                            'index.html<i class="close fa fa-times" onclick="deleteSelectedEditorTab()"></i>'
                        );
                    } else {
                        $("div.tab.selected").html(
                            'Tab 1<i class="close fa fa-times" onclick="deleteSelectedEditorTab()"></i>'
                        );
                    }
                    sizeTabs();
                } else {
                    // find nearest tab to the left... unless we're the first one, in which case to the right...
                    var nearest = $("#code-titlebar div.tab.selected").prev(
                        "div.tab"
                    );
                    if (nearest.length == 0)
                        nearest = $("#code-titlebar div.tab.selected").next(
                            "div.tab"
                        );
...
function populateTabs(data, noremove) {
    ...
    $.each(data, function (i, tab) {
        var newtab = $(
            '<div onclick="selectEditorTab(this)" class="tab">' +
                tab[0].trim() +
                '<i class="close fa fa-times" onclick="deleteSelectedEditorTab()"></i></div>'
        );
        // put code in existing tab into a hidden pre.code
        var newpre = $('<pre class="code"/>');
    ...
    if (!$("#code-titlebar div.tab").eq(0).hasClass("selected"))
...
function sizeTabs() {
    $(".tab")
        ...
        .css(
            "max-width",
            $("#code-titlebar").width() / $(".tab").not(".newtab").length + "px"
        );
}
...
function submitWork(source, oneOnly) {
    ...
    setTimeout(function () {
        ...
        if (oneOnly) {
            ...
            $(source).text(
                ">>> You have already submitted this, but can submit it multiple times. Click here to submit again. <<<"
            );
        }
    }, 1000);
}
...
function validXHTML() {
    ...
    setTimeout(function () {
        var loc = encodeURIComponent(
            $("iframe#outputframe")[0].contentWindow.location.href
        );
        $("iframe#outputframe")[0].src =
            "http://validator.w3.org/check?uri=" + loc;
    }, 2000);
}
...
function logout() {
    ...
    setInterval(function () {
        window.location = contextPath + "/Login?logout=true";
    }, 2000);
}
...
function addNewTab(auto) {
    if ($("div.parameter#language").text().trim() == "fullweb" && !auto) {
        var tabName = "";
        var msg =
            "What do you want to call the new tab? Include the file extension." +
        ...
        var tabNos = 1;
        while (
            $("#code-titlebar div:contains(Tab " + tabNos + ")").length != 0
        ) {
            tabNos++;
        }
        tabName = "Tab " + tabNos;
...
function cheatSource(code) {
    ...
    for (var x = 0; x < lines.length; x++) {
        ...
        if (res) {
            ...
            console.log(source);
            sourceCounts[source]++;
            if (source != strippedKno && sourceCounts[source] > 1)
                return source; // cheat and more than one match!
        }
    }
    return false;
}
...
function blocklyLoaded(blockly) {
    ...
    $("#code-blockly")
        .contents()
        .find("body div.blocklyToolboxDiv")
        .append('<div id="blocklyCodePreviewTitlebar">Code Preview</div>');
    $("#code-blockly")
        .contents()
        .find("body div.blocklyToolboxDiv")
        .append('<div id="blocklyCodePreview"></div>');
}
...
function blocklyCodeUpdate() {
    if ($("div.parameter#blockly").text().trim() != "true") return;
    var $blocklyCodePreview = $("#code-blockly")
        .contents()
        .find(".blocklyToolboxDiv #blocklyCodePreview");

    var originalCode = getCode($blocklyCodePreview)
        .replace(/[0-9]+/, "")
        .replace(/\n[0-9]+/g, "\n");
    ...
    for (var i = 0; i < splitted.length; i++) {
        ...
        bcp +=
            '<div id="bcp' +
            i +
            '" unselectable="on"><div class="cblineno">' +
            (i + 1) +
            "</div>" +
            splitted[i] +
            "</div>\n";
    ...
    var newCode = getCode($blocklyCodePreview)
        .replace(/[0-9]+/, "")
        .replace(/\n[0-9]+/g, "\n");
...
function restoreBlockly(oldBlockly) {
    if (Blockly == null)
        Blockly = $("iframe#code-blockly").get(0).contentWindow.Blockly;
    Blockly.mainWorkspace.clear();
    ...
    Blockly.Xml.domToWorkspace(
        ...
        Blockly.Xml.textToDom(oldBlockly)
    );
    blocklyCodeUpdate();
}
...
function javaPidjinCodeWrapper() {
    if (
        $("div.parameter#language").text().trim() != "java" ||
        $("div.parameter#multi").text().trim() == "true"
    )
        return;
    if (jpHeader != null) jpHeader.clear();
    if (jpFooter != null) jpFooter.clear();
    jpHeader = editor.addLineWidget(
        0,
        $(
            '<pre class="javastatic top">import java.util.Scanner;\n\npublic class SomeJavaCode\n{\n  public static void main(String[] args)\n  {</pre>'
        ).get(0),
        { above: true, coverGutter: true }
    );
    jpFooter = editor.addLineWidget(
        editor.lineCount(),
        $(
            '<pre class="javastatic bottom">  } // end of main method\n} // end of class</pre>'
        ).get(0),
        { coverGutter: true }
    );
    editor.refresh();
...
function toggleBlocks() {
    ...
    if (cookieStatus != "true") {
        var msg =
            "This will disable the blocks and enable the text-based coding. You must save your work before you proceed.<p>";
        ...
        var msg =
            "This will disable the text-based coding and enable blocks for relevant exercises. You must save your work before you proceed.<p>";
        msg += "Are you sure you want to proceed?";
        apprise(msg, { verify: true }, function (r) {
            if (r) {
...
function toggleGraphics(turnon) {
    ...
    $("div#navchat,div#navlecture,div#navcourse,div#navvideo").removeClass(
        "selected"
    );
    if (turnon) {
...
function addGettersAndSetters(which) {
    ...
    try {
        ...
        if (attributeLines.trim() == "") {
            apprise(
                "You need to highlight the attributes you want to create getters and/or setters for."
            );
            return;
        }
        var attributeLines = attributeLines.match(
            /private ([\w|\<|\>]*) (\w*);|private ([\w|\<|\>]*) (\w*)\s*=/gm
        );
        ...
        for (var i = 0; i < attributeLines.length; i++) {
            ...
            var details = attributeLine.match(
                /private ([\w|\<|\>|\[|\]]*) (\w*);/
            );
            if (details == null)
                details = attributeLine.match(
                    /private ([\w|\<|\>|\[|\]]*) (\w*)\s*=/
                );
            ...
            if (which == "setters" || which == "both") {
                ...
                newcode += indent + "{\n";
                newcode +=
                    indent + "   this." + varname + " = " + varname + ";\n";
                newcode += indent + "}\n\n";
            }
            if (which == "getters" || which == "both") {
                var methodprefix =
                    dataType.toLowerCase() == "boolean" ? "is" : "get";
...
window.onload = function () {
    ...
    /*else // if Java, handle runtime selection
   {
    ...
    if ($("div.parameter#language").text().trim() == "basic") {
        ...
        $("div.parameter#language").text().trim().indexOf("python") != -1
    ) {
    ...
    if ($("div.parameter#language").text().trim() == "java") {
        if (newdoppio) {
            ...
            $("div.javafs-file").each(function () {
                ...
                extra
                    .find("b")
                    .text(
                        "Click here to upload example files into your Java file system >>>"
                    );
                extra.append(
                    '<img src="' + contextPath + '/images/bundle_icon.png"/>'
                );
                $(this).prepend(extra);
            });
            // load doppio
        ...
        if ($.browser.msie) {
            alert(
                "You are using Internet Explorer, in which this material will not function properly. You will " +
    ...
    if (
        $("div.parameter#language").text().trim() == "pcode" ||
        $("div.parameter#language").text().trim() == "basic" ||
        $("div.parameter#language").text().trim() == "pcarol"
    ) {
    ...
    if (
        ...
        $("#code-titlebar").append(
            '<div class="tab newtab" onclick="addNewTab()">+</div>'
        );
    ...
    if ($("div.parameter#videoUrl").length != 0) {
        ...
        $("div#topnav div.row1").append(
            '<div title="Show lecture video" id="navvideo"><i class="fa fa-video-camera"></i></div>'
        );
        $("body").append(
            '<div id="video"><iframe data-loaded="false" src="' +
                contextPath +
                '/holding.html"></iframe></div>'
        );
        $("div#navvideo").click(function () {
            $(
                "div#lectureslides,div#coursenavpage,div#chat,div#graphics"
            ).hide();
            $(
                "div#navlecture,div#navcourse,div#navchat,div#navgraphics"
            ).removeClass("selected");
            $("div#video").toggle();
            if ($("div#video iframe").attr("data-loaded") == "false") {
                $("div#video iframe").attr(
                    "src",
                    $("div.parameter#videoUrl").text().trim()
                );
    ...
    if ($("div.parameter#lectureSlideUrl").length != 0) {
        ...
        $("div#topnav div.row1").append(
            '<div title="Show lecture slides" id="navlecture"><i class="fa fa-graduation-cap"></i></div>'
        );
        $("body").append(
            '<div id="lectureslides"><iframe data-loaded="false" src="' +
                contextPath +
                '/holding.html"></iframe></div>'
        );
        $("div#navlecture").click(function () {
            ...
            $(
                "div#navvideo,div#navcourse,div#navchat,div#navgraphics"
            ).removeClass("selected");
            $("div#lectureslides").toggle();
            if ($("div#lectureslides iframe").attr("data-loaded") == "false") {
                $("div#lectureslides iframe").attr(
                    "src",
                    $("div.parameter#lectureSlideUrl").text().trim()
                );
    ...
    if ($("div.parameter#courseNavUrl").length != 0) {
        ...
        $("div#topnav div.row1").append(
            '<div title="Show course navigation" id="navcourse"><i class="fa fa-globe"></i></div>'
        );
        $("body").append(
            '<div id="coursenavpage"><iframe data-loaded="false" src="' +
                contextPath +
                '/holding.html"></iframe></div>'
        );
        $("div#navcourse").click(function () {
            ...
            $(
                "div#navvideo,div#navlecture,div#navchat,div#navgraphics"
            ).removeClass("selected");
            $("div#coursenavpage").toggle();
            if ($("div#coursenavpage iframe").attr("data-loaded") == "false") {
                $("div#coursenavpage iframe").attr(
                    "src",
                    $("div.parameter#courseNavUrl").text().trim()
                );
    ...
    if ($("div.parameter#chatUrl").length != 0) {
        ...
        $("div#topnav div.row1").append(
            '<div title="Show chat pane" id="navchat"><i class="fa fa-commenting-o"></i></div>'
        );
        $("body").append(
            '<div id="chat"><iframe data-loaded="false" src="' +
                contextPath +
                '/holding.html"></iframe></div>'
        );
        $("div#navchat").click(function () {
            $(
                "div#lectureslides,div#coursenavpage,div#video,div#graphics"
            ).hide();
            $(
                "div#navlecture,div#navcourse,div#navvideo,div#navgraphics"
            ).removeClass("selected");
            $("div#chat").toggle();
            if ($("div#chat iframe").attr("data-loaded") == "false") {
                ...
                if (chaturl.indexOf("{course}") != -1) {
                    chaturl = chaturl.replace(
                        "{course}",
                        $("div.parameter#courseNo").text().trim()
                    );
                }
                ...
                if (chaturl.indexOf("{lessonno}") != -1) {
                    chaturl = chaturl.replace(
                        "{lessonno}",
                        $("div.parameter#lessonNo").text().trim()
                    );
                }
    ...
    if (getUrlParameter("embedmedal")) {
        ...
        $("input#tidy").hide();
        if ($("pre.codepaste").size() != 0 || $("div.codebundle").size() != 0)
            $("input#pasteExample").show();
        $("input#runMedal").show();
        ...
        $("div#editorRightClick").css(
            "left",
            $("input#gettersAndSetters").position().left +
                20 +
                $("input#gettersAndSetters").width() -
                $("div#editorRightClick").width() +
                "px"
        );
    ...
    var courseNo = $("#courseNo.parameter").text().trim();
    var lessonNo = $("#lessonNo.parameter").text().trim();
    var lastSectionNo = $.cookie(
        "nlpp-" + courseNo + "-" + lessonNo + "-lastnav"
    );
    var oldCode = $.cookie("nlpp-" + courseNo + "-" + lessonNo + "-code");
    var oldBlockly = $.cookie(
        "nlpp-" + courseNo + "-" + lessonNo + "-codeblockly"
    );
    ...
    if (embed) {
        ...
        if (carolcode) {
            ...
            if (carolcode.trim().slice(0, 8) == "fakeDoc:") {
                ...
                var newiframe = $(
                    '<iframe style="border: 0px; height: 100%; width: 100%" border="0"><iframe>'
                );
    ...
    if ($("div.parameter#kinder").text().trim() != "true" && !embed) {
        if (
            (lastSectionNo != null &&
                lastSectionNo != 0 &&
                lastSectionNo.length != 0) ||
            oldCode != ""
        ) {
            if (isNaN(lastSectionNo) || lastSectionNo.length == 0)
                lastSectionNo = 0;
            clearInterval(saveInterval);
            var msg =
                "Do you want to resume from where you last left this lesson?";
            if (hash != 0)
                msg =
                    "Do you want to restore the code you had when you were last on this lesson?";
            apprise(msg, { verify: true }, function (r) {
                if (r) {
                    ...
                    if (
                        $("div.parameter#blockly").text().trim() == "true" &&
                        oldBlockly != ""
                    ) {
    ...
    /*if ($.browser.msie)
   {
    ...
    function copyCut(editor, e) {
        ...
        if (navigator.platform.slice(0, 3) == "Mac") {
            ...
            $("body").append(
                '<textarea style="position: fixed; top: 50px; left: 10px; width: 100px; height: 100px" id="tempcopy"></textarea>'
            );
    ...
    editor.on("paste", function (editor, e) {
        ...
        if (
            navigator.platform.slice(0, 3) == "Mac" &&
            window.NLCB != undefined
        ) {
            // DIE, Apple. Just f**king DIE.
    ...
    if (
        ...
        if (
            $("div.parameter#gettersAndSetters").text().trim() != "" ||
            getKNo().indexOf("ku") != -1
        ) {
            ...
            $("div#code-main").mousedown(function (e) {
                ...
                if (e.button == 2) {
                    ...
                    setTimeout(function () {
                        $("div#editorRightClick").show();
                    }, 200);
                    return false;
                }
                return true;
            });
        } // but if not, disable the G&S button when embedded
        else {
            console.log(getKNo());
    ...
    $.get(contextPath + "/slackers.txt", function (data) {
        var x = "";
        for (var i = 32; i < watermark.length; i = i + 33)
            x += watermark.charAt(i);
        if (data.indexOf(x) != -1) {
    ...
    if (
        ...
        $("div#code-titlebar").html(
            '[ Code <i onclick="toggleBlocks()" class="fa fa-cube" style="cursor:pointer; color: red;" aria-hidden="true"></i> ]'
        );
    }
    ...
    if ($("div.parameter#blockly").text().trim() == "true") {
        ...
        if (!$("body").hasClass("altstyle")) {
            ...
            $(
                "head",
                $("iframe#code-blockly")[0].contentWindow.document
            ).append(
                '<link rel="stylesheet" href="' +
                    $("div.parameter#altstyle").text().trim() +
                    '"/>'
            );
        }
        ...
        $("div#usermenu div#toggleblocks").removeClass("disabled");
        $("div#code-titlebar").html(
            '[ Code <i onclick="toggleBlocks()" class="fa fa-cube" style="cursor: pointer; color: green;" aria-hidden="true"></i> ]'
        );
    ...
    $("img[src$=gif]").each(function () {
        ...
        sg.load(function () {
            ...
            $(div).append(
                '<div class="pntoolbar" style="position: absolute; right: 3px; bottom : 3px;"><button style="height: 30px; width: 30px; padding: 0px; line-height: 30px; border-radius: 15px; cursor: pointer; text-align: center; outline: 0px"><i class="fa fa-pause"></i></button>'
            );
    ...
    $("div.section").each(function (index) {
        $(this).append(
            '<br/><div class="internalsectionnav"><a href="javascript:void(0)" class="next" style="float: right">Next Section &gt;&gt;</a><a href="javascript:void(0)" class="prev">&lt;&lt; Previous Section</a></div><br/><br/>'
        );
        $(this)
            .find("a.next")
            .click(function () {
    ...
    $("div#output-titlebar").mousedown(function () {
        ...
        if (
            $("div.parameter#language").text().trim() == "fullweb" &&
            $("div#editor-wrapper").hasClass("maxed")
        )
            return;
        // otherwise...
        // create a big div over the top of everything
        $("body").append(
            '<div id="resizeoverlay" style="position: fixed; top: 0px; bottom: 0px; left: 0px; right: 0px; z-index: 20000;"></div>'
        );
        ...
        dragY = setInterval(function () {
            ...
            if (!$("body").hasClass("altstyle")) {
                $("div#output-outer").css(
                    "height",
                    window.innerHeight - globalMouseY + "px"
                );
                $("div#editor-wrapper").css(
                    "bottom",
                    window.innerHeight - globalMouseY + "px"
                );
    ...
    if ($("div.parameter#interactionlog").text().trim() != "")
        interactionlog = $("div.parameter#interactionlog").text().trim();
    if ($("div.parameter#interactionlogitems").text().trim() != "")
        interactionlogitems = $("div.parameter#interactionlogitems")
            .text()
            .trim();
...
function getEmbedLink() {
    ...
    if ($("div.fakedocwrapper.onscreen").size() == 1)
        data.fakeDoc = $("body", visibleFakeDoc).html();
    $.post(contextPath + "/GenerateEmbed", data, function (id) {
        var msg =
            "To embed your work in another page, use the following HTML code:<p>";
        msg +=
            '<textarea style="width: 100%; padding: 8px; box-sizing: border-box">&lt;iframe style="width: 500px; height: 500px;" src="' +
            location.origin +
            contextPath +
            "?embed=" +
            id +
        ...
        msg +=
            "<p>You can tweak the <code>width</code> and <code>height</code> to suit the host page if needed.</p>";
        apprise(msg);
    });
}
...
function mainZoom() {
    ...
    if ($("div#mainZoomControls").is(":hidden")) {
        ...
        $(document).mouseup(function (e) {
            ...
            if (
                e.target.id != "newzoom" &&
                $(e.target).parents("div#topnav").length == 0
            ) {
